...
if(isset($_POST['del_proj']))
{
	 ...
	 $delProjval=$_POST['del_val'];
	  ...
	  $deleteProj="UPDATE office SET trash=$delProjval WHERE idoficina=$delProjId";
	  $projDeleted=mysqli_query($connect, $deleteProj);
	  if($projDeleted){
		 header("Location:oficina.php?message=psuccess");
	   } else{
...
$deleteProj="UPDATE office SET trash=$bulk_del_val WHERE idoficina=$bulk_id";
...
if(isset($_POST['comp_proj']))
{
	 ...
	 if($comp_val == 1){
	  ...
	  if($comp_proj){
		  header("Location:oficina.php?message=completed");
	  } else{
		  header("Location:oficina.php?message=fail");
	  }
	 } else{
	  ...
	  $comp_proj=mysqli_query($connect, $updateProj);
	  if($comp_proj){
		  header("Location:oficina.php?message=reopen");
	  } else{
		  header("Location:oficina.php?message=fail");
	  }		 
	 }
...
if(isset($_POST['arc_proj']))
{
	 ...
	 $arc_val=$_POST['arc_val'];
	  $updateProj="UPDATE office SET archive=$arc_val WHERE idoficina=$arc_id";
	  $arc_proj=mysqli_query($connect, $updateProj);
	  if($arc_proj){
		  header("Location:oficina.php?message=archive");
	  }else{
		   header("Location:oficina.php?message=fail");
	  }
}
...
<?php include('../templates/top-header.php'); ?>
         <div class="row">
			...
			<div class="row project-dash">
				...
				<div class="pm-heading"><h2><?php echo $lang['Manage office']; ?> </h2><span><?php echo $lang['All office leads']; ?></span></div>
                      ...
                      foreach($recentProjects as $recentProject){ ?>
                            ...
                            <td class="tbl-ttl"><span class="onmobile"><?php echo $lang['Title']; ?>: </span> <?php echo $recentProject->name;?></td>
							  ...
							  <ul>
								...
								<li>
								<form method="post" action="edit-oficina.php">
								<input type="hidden" value="<?php echo $recentProject->idoficina;?>" name="idoficina"/>
								<button type="submit" name="edt_pro"><i class="fa fa-pencil"></i> <?php echo $lang['Edit office']; ?></button>
								</form>
								</li>
								<li>
								<form method="post" action="#">
