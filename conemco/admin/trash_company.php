...
if(isset($_POST['del_proj']))
{
	 $delProjId=$_POST['del_id'];
	  ...
	  $deleteProj="delete from companies where idcompany=$delProjId limit 1";
	  $projDeleted=mysqli_query($connect, $deleteProj);
	  $projChatDeleted=mysqli_query($connect, $deleteProjChat);
	  if($projDeleted){
		 header("Location:trash_company.php?message=trash");
	   } else{
			   header("Location:trash_company.php?message=fail");
	   }
 }
if(isset($_POST['comp_proj']))
{
	 ...
	 $comp_val=$_POST['comp_val'];
	  $updateProj="UPDATE companies SET status=$comp_val WHERE idcompany=$comp_id";
	  $comp_proj=mysqli_query($connect, $updateProj);
if($comp_proj){
		  header("Location:Company.php?message=restore");
	  } else{
...
if(isset($_POST['arc_proj']))
{
	 ...
	 $arc_del_val=$_POST['arc_del_val'];
	  $updateProj="UPDATE companies SET trash=$arc_del_val WHERE idcompany=$arc_id";
...
$notmsgb = $lang['company has been created successfully!']; 
$notmsgc = $lang['Error! Please Try Again later.'];
$notmsgd = $lang['company has been deleted sucessfully'];
$notmsgg = $lang['company status updated to re-open.'];
$notmsgh = $lang['company restored Successfully.'];
...
<?php include('../templates/top-header.php'); ?>
         <div class="row">
			...
			<div class="row project-dash">
				...
				<div class="pm-heading"><h2><?php echo $lang['Trash']; ?> </h2><span><?php echo $lang['All company deleted']; ?></span></div>
				<div class="pm-form"><form class="form-inline md-form form-sm">
    <input class="form-control form-control" type="text" placeholder="<?php echo $lang['Search company']; ?>" id="protbl-input">
    <button type="submit"><i class="fa fa-search" aria-hidden="true"></i></button>
</form></div>
<?php $recentProjects=company::findBySql("select * from companies WHERE trash=1");?>
				</div> 
                <div class="col-lg-7 col-md-7 col-sm-7 project-opt creative-right text-right">
				<div class="mobilepromenu">
				<div class="projbtnm"><a href="add-company.php"><span>+</span></a></div>
				...
				<li class="cproject"><a href="add-company.php"><?php echo $lang['Create company']; ?> <span>+</span></a></li>
				<li class="pm-arc"><a href="Company.php"><?php echo $lang['Company']; ?></a></li>
				<li class="pm-trashbox"><a href="trash_company.php">
				<?php echo $lang['TRASH']; ?> (<?php echo count($recentProjects); ?>)
				</a></li>
				</ul>
			</div>
...
$recentProjects_page=company::findBySql("select * from companies WHERE trash = 1"); 
$recentProjects=company::findBySql("select * from companies WHERE trash = 1 ORDER BY date_created DESC LIMIT $start_from, $limit"); 
                ...
                <table class="table table-new projectspage" data-pagination="true" data-page-size="5">
                        <thead>
                          <tr>
                            ...
                            <th width="26%"><?php echo $lang['Company Name']; ?></th> 
                            <th><?php echo $lang['Description company']; ?></th>
					...
					if($recentProjects == NULL){
                      ...
                      foreach($recentProjects as $recentProject){ ?>
                            ...
                            <tr>
							<td class="bs-checkbox" style="width:36px !important; text-align:left !important;"><input data-index="<?php echo $counter; ?>" value="<?php echo $recentProject->idcompany ?>" name="btSelectItem" type="checkbox"></td>
                            <td class="tbl-ttl"><span class="onmobile"><?php echo $lang['Title']; ?>: </span> <?php echo $recentProject->name;?></td>
							  ...
							  <div class="action-toggle" data-toggle="collapse" data-target="#client-menu<?php echo $recentProject->idcompany;?>"><?php echo $lang['Action']; ?> <i class="fa fa-caret-down"></i></div>
							  <div id="client-menu<?php echo $recentProject->idcompany;?>" class="toggle-action collapse">
							  <ul>
								<li>
								<form method="post" action="#">
								<input type="hidden" value="<?php echo $recentProject->idcompany;?>" name="arc_id"/>
								...
								<li>
								<button type="button" data-toggle="modal" class="mod" data-target="#deletepro<?php echo $recentProject->idcompany;?>"><i class="fa fa-trash-o"></i> <?php echo $lang['Delete Permanently']; ?></button>
								</li>
							</ul>
								</div>
							...
							<!-- The Modal -->
<div  class="modal hide fade deletepro"  tabindex="-1" aria-labelledby="basicModal" aria-hidden="true" id="deletepro<?php echo $recentProject->idcompany;?>" role="dialog">
...
<div class="mod-footbox">
<form method="post" action="#">
								<input type="hidden" value="<?php echo $recentProject->idcompany;?>" name="del_id"/>
...
$(document).ready(function() {
	$(".mod").click(function(){
		$('#deletepro<?php echo $recentProject->idcompany;?>').modal('toggle');
		$("#deletepro<?php echo $recentProject->idcompany;?>").removeClass("fade");
