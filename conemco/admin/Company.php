...
if(isset($_POST['del_proj']))
{
	 ...
	 $delProjval=$_POST['del_val'];
	  ...
	  $deleteProj="UPDATE companies SET trash=$delProjval WHERE idcompany=$delProjId";
	  $projDeleted=mysqli_query($connect, $deleteProj);
	  if($projDeleted){
		 header("Location:Company.php?message=psuccess");
	   } else{
...
$deleteProj="UPDATE companies SET trash=$bulk_del_val WHERE idcompany=$bulk_id";
...
if(isset($_POST['comp_proj']))
{
	 ...
	 if($comp_val == 1){
	  ...
	  if($comp_proj){
		  header("Location:Company.php?message=completed");
	  } else{
		  header("Location:Company.php?message=fail");
	  }
	 } else{
	  ...
	  $comp_proj=mysqli_query($connect, $updateProj);
	  if($comp_proj){
		  header("Location:Company.php?message=reopen");
	  } else{
		  header("Location:Company.php?message=fail");
	  }		 
	 }
...
if(isset($_POST['arc_proj']))
{
	 ...
	 $arc_val=$_POST['arc_val'];
	  $updateProj="UPDATE companies SET archive=$arc_val WHERE idcompany=$arc_id";
	  $arc_proj=mysqli_query($connect, $updateProj);
	  if($arc_proj){
		  header("Location:Company.php?message=archive");
	  }else{
		   header("Location:Company.php?message=fail");
	  }
}
...
<?php include('../templates/top-header.php'); ?>
         <div class="row">
			...
			<div class="row project-dash">
				...
				<div class="pm-heading"><h2><?php echo $lang['Manage company']; ?> </h2><span><?php echo $lang['All company leads']; ?></span></div>
...
<li class="cproject deskvisibleb"><a href="add-company.php"><?php echo $lang['Create company']; ?> <span>+</span></a></li>
<li class="cproject tabvisible"><a href="add-company.php"><span>+</span></a></li>
<li class="pm-trashbox"><a href="trash_company.php">
				<?php echo $lang['TRASH']; ?> (<?php echo count($recentProjects); ?>)
				</a></li>
				<li class="pm-trash">
                      ...
                      foreach($recentProjects as $recentProject){ ?>
                            ...
                            <td class="tbl-ttl"><span class="onmobile"><?php echo $lang['Title']; ?>: </span> <?php echo $recentProject->name;?></td>
							  ...
							  <ul>
								...
								<input type="hidden" value="<?php echo $recentProject->idcompany;?>" name="idcompany"/>
								<button type="submit" name="edt_pro"><i class="fa fa-pencil"></i> <?php echo $lang['Edit Company']; ?></button>
								</form>
								</li>
								<li>
								<form method="post" action="#">
