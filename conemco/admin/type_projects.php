...
if(isset($_POST['del_proj']))
{
	 ...
	 $delProjval=$_POST['del_val'];
	  ...
	  $deleteProj="UPDATE project_types SET trash=$delProjval WHERE idtype=$delProjId";
	  $projDeleted=mysqli_query($connect, $deleteProj);
	  if($projDeleted){
		 header("Location:type_projects.php?message=psuccess");
	   } else{
			   header("Location:type_projects.php?message=fail");
	   }
}
...
$deleteProj="UPDATE project_types SET trash=$bulk_del_val WHERE idtype=$bulk_id";
...
if(isset($_POST['comp_proj']))
{
	 ...
	 if($comp_val == 1){
	  $updateProj="UPDATE project_types SET status=$comp_val WHERE idtype=$comp_id";
	  $comp_proj=mysqli_query($connect, $updateProj);
	  if($comp_proj){
		  header("Location:type_projects.php?message=completed");
	  } else{
		  header("Location:type_projects.php?message=fail");
	  }
	 } else{
	  ...
	  $comp_proj=mysqli_query($connect, $updateProj);
	  if($comp_proj){
		  header("Location:type_projects.php?message=reopen");
	  } else{
		  header("Location:type_projects.php?message=fail");
	  }		 
	 }
...
if(isset($_POST['arc_proj']))
{
	 ...
	 $arc_val=$_POST['arc_val'];
	  $updateProj="UPDATE project_types SET archive=$arc_val WHERE idtype=$arc_id";
	  $arc_proj=mysqli_query($connect, $updateProj);
	  if($arc_proj){
		  header("Location:project_types.php?message=archive");
	  }else{
		   header("Location:project_types.php?message=fail");
	  }
}
...
<?php include('../templates/top-header.php'); ?>
         <div class="row">
			...
			<div class="row project-dash">
				...
				<div class="pm-heading"><h2><?php echo $lang['Manage type']; ?> </h2><span><?php echo $lang['All type leads']; ?></span></div>
				<div class="pm-form"><form class="form-inline md-form form-sm">
    <input class="form-control form-control" type="text" placeholder="<?php echo $lang['Search type']; ?>" id="protbl-input">
    <button type="submit"><i class="fa fa-search" aria-hidden="true"></i></button>
</form></div>
<?php $recentProjects=Type::findBySql("select * from project_types WHERE trash=1");?>
				</div> 
                <div class="col-lg-7 col-md-7 col-sm-7 project-opt creative-right text-right">
				<div class="mobilepromenu">
				<div class="projbtnm"><a href="add-new-type.php"><span>+</span></a></div>
				<div class="projmobilem">
				<i class="fa fa-bars" aria-hidden="true"></i>
					<ul>
					<li class="pm-trashbox"><a href="trash_type_project.php">
...
<li class="cproject deskvisibleb"><a href="add-new-type.php"><?php echo $lang['Create type']; ?> <span>+</span></a></li>
<li class="cproject tabvisible"><a href="add-new-type.php"><span>+</span></a></li>
<li class="pm-trashbox"><a href="trash_type_project.php">
				<?php echo $lang['TRASH']; ?> (<?php echo count($recentProjects); ?>)
				</a></li>
				<li class="pm-trash">
...
$recentProjects_page=Type::findBySql("select * from project_types WHERE trash != 1"); 
$recentProjects=Type::findBySql("select * from project_types WHERE trash != 1 ORDER BY date_created DESC LIMIT $start_from, $limit"); 
                      ...
                      foreach($recentProjects as $recentProject){ ?>
                            ...
                            <td class="tbl-ttl"><span class="onmobile"><?php echo $lang['Title']; ?>: </span> <?php echo $recentProject->idtype;?></td>
							  ...
							  <ul>
								...
								<li>
								<form method="post" action="edit-type_projects.php">
								<input type="hidden" value="<?php echo $recentProject->idtype;?>" name="idtype"/>
								<button type="submit" name="edt_pro"><i class="fa fa-pencil"></i> <?php echo $lang['Edit type']; ?></button>
								</form>
								</li>
								<li>
								<form method="post" action="#">
