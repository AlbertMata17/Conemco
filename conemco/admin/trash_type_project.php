...
if(isset($_POST['del_proj']))
{
	 $delProjId=$_POST['del_id'];
	  ...
	  $deleteProj="delete from project_types where idtype=$delProjId limit 1";
	  $projDeleted=mysqli_query($connect, $deleteProj);
	  $projChatDeleted=mysqli_query($connect, $deleteProjChat);
	  if($projDeleted){
		 header("Location:trash_type_project.php?message=trash");
	   } else{
			   header("Location:trash_type_project.php?message=fail");
	   }
 }
...
if($comp_proj){
		  header("Location:type_projects.php?message=restore");
	  } else{
...
if(isset($_POST['arc_proj']))
{
	 ...
	 $arc_del_val=$_POST['arc_del_val'];
	  $updateProj="UPDATE project_types SET trash=$arc_del_val WHERE idtype=$arc_id";
...
<?php $recentProjects=Type::findBySql("select * from project_types WHERE trash=1");?>
				...
				<li class="cproject"><a href="add-new-type.php"><?php echo $lang['Create type']; ?> <span>+</span></a></li>
				<li class="pm-arc"><a href="type_projects.php"><?php echo $lang['Type']; ?></a></li>
				<li class="pm-trashbox"><a href="trash_type_project.php">
				<?php echo $lang['TRASH']; ?> (<?php echo count($recentProjects); ?>)
				</a></li>
				</ul>
