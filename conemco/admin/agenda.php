...
if(isset($_POST['del_proj']))
{
	 ...
	 $delProjval=$_POST['del_val'];
	  ...
	  $deleteProj="UPDATE customers SET trash=$delProjval WHERE idcustomer=$delProjId";
	  $projDeleted=mysqli_query($connect, $deleteProj);
	  if($projDeleted){
		 header("Location:agenda.php?message=psuccess");
	   } else{
...
$deleteProj="UPDATE customers SET trash=$bulk_del_val WHERE idcustomer=$bulk_id";
...
if(isset($_POST['comp_proj']))
{
	 ...
	 if($comp_val == 1){
	  ...
	  if($comp_proj){
		  header("Location:agenda.php?message=completed");
	  } else{
		  header("Location:agenda.php?message=fail");
	  }
	 } else{
	  ...
	  $comp_proj=mysqli_query($connect, $updateProj);
	  if($comp_proj){
		  header("Location:agenda.php?message=reopen");
	  } else{
		  header("Location:agenda.php?message=fail");
	  }		 
	 }
...
if(isset($_POST['arc_proj']))
{
	 ...
	 $arc_val=$_POST['arc_val'];
	  $updateProj="UPDATE customers SET archive=$arc_val WHERE idcustomer=$arc_id";
	  $arc_proj=mysqli_query($connect, $updateProj);
	  if($arc_proj){
		  header("Location:agenda.php?message=archive");
	  }else{
		   header("Location:agenda.php?message=fail");
	  }
}
...
$notmsgb = $lang['contact has been created successfully!']; 
$notmsgc = $lang['Error! Please Try Again later.'];
$notmsgd = $lang['contact has been deleted sucessfully'];
$notmsgg = $lang['contact status updated to re-open.'];
$notmsgh = $lang['contact restored Successfully.'];
...
<?php include('../templates/top-header.php'); ?>
         <div class="row">
			...
			<div class="row project-dash">
				...
				<div class="pm-heading"><h2><?php echo $lang['Manage contact']; ?> </h2><span><?php echo $lang['All contact leads']; ?></span></div>
				<div class="pm-form"><form class="form-inline md-form form-sm">
    <input class="form-control form-control" type="text" placeholder="<?php echo $lang['Search contact']; ?>" id="protbl-input">
...
<li class="cproject deskvisibleb"><a href="add-new-agenda.php"><?php echo $lang['Create Contact']; ?> <span>+</span></a></li>
<li class="cproject tabvisible"><a href="add-new-agenda.php"><span>+</span></a></li>
<li class="pm-trashbox"><a href="trash_agenda.php">
				<?php echo $lang['TRASH']; ?> (<?php echo count($recentProjects); ?>)
				</a></li>
				<li class="pm-trash">
                ...
                <table class="table table-new projectspage" data-pagination="true" data-page-size="5">
                        <thead>
                          <tr>
                            ...
                            <th width="26%"><?php echo $lang['contact Name']; ?></th> 
					...
					if($recentProjects == NULL){
						  echo '<tr><td colspan="9">'.$lang['There is no contact Please Create contact!'].'</td></tr>';
					  }
                      foreach($recentProjects as $recentProject){ ?>
                            ...
                            <td class="tbl-ttl"><span class="onmobile"><?php echo $lang['Title']; ?>: </span> <?php echo $recentProject->name;?></td>
							  ...
							  <div class="action-toggle" data-toggle="collapse" data-target="#client-menu<?php echo $recentProject->idcustomer;?>"><?php echo $lang['Action']; ?><i class="fa fa-caret-down"></i></div>
							  <div id="client-menu<?php echo $recentProject->idcustomer;?>" class="toggle-action collapse">
							  <ul>
								
								<li>
								<form method="post" action="edit-agenda.php">
								<input type="hidden" value="<?php echo $recentProject->idcustomer;?>" name="idcustomer"/>
								<button type="submit" name="edt_pro"><i class="fa fa-pencil"></i> <?php echo $lang['Edit contact']; ?></button>
								</form>
								</li>
								<li>
								<form method="post" action="#">
