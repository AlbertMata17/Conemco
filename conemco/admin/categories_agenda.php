...
if(isset($_POST['del_proj']))
{
	 ...
	 $delProjval=$_POST['del_val'];
	  ...
	  $deleteProj="UPDATE categories_diary SET trash=$delProjval WHERE idtype=$delProjId";
	  $projDeleted=mysqli_query($connect, $deleteProj);
	  if($projDeleted){
		 header("Location:categories_agenda.php?message=psuccess");
	   } else{
...
$deleteProj="UPDATE categories_diary SET trash=$bulk_del_val WHERE idtype=$bulk_id";
...
if(isset($_POST['comp_proj']))
{
	 ...
	 if($comp_val == 1){
	  ...
	  if($comp_proj){
		  header("Location:categories_agenda.php?message=completed");
	  } else{
		  header("Location:categories_agenda.php?message=fail");
	  }
	 } else{
	  ...
	  $comp_proj=mysqli_query($connect, $updateProj);
	  if($comp_proj){
		  header("Location:categories_agenda.php?message=reopen");
	  } else{
		  header("Location:categories_agenda.php?message=fail");
	  }		 
	 }
...
if(isset($_POST['arc_proj']))
{
	 ...
	 $arc_val=$_POST['arc_val'];
	  $updateProj="UPDATE categories_diary SET archive=$arc_val WHERE idtype=$arc_id";
	  $arc_proj=mysqli_query($connect, $updateProj);
	  if($arc_proj){
		  header("Location:categories_agenda.php?message=archive");
	  }else{
		   header("Location:categories_agenda.php?message=fail");
	  }
}
...
<?php include('../templates/top-header.php'); ?>
         <div class="row">
			...
			<div class="row project-dash">
				...
				<div class="pm-heading"><h2><?php echo $lang['Manage type diary']; ?> </h2><span><?php echo $lang['All type leads']; ?></span></div>
...
<li class="cproject deskvisibleb"><a href="add-new-categories-agenda.php"><?php echo $lang['Create type']; ?> <span>+</span></a></li>
<li class="cproject tabvisible"><a href="add-new-categories-agenda.php"><span>+</span></a></li>
<li class="pm-trashbox"><a href="trash_categories.php">
				<?php echo $lang['TRASH']; ?> (<?php echo count($recentProjects); ?>)
				</a></li>
				<li class="pm-trash">
                      ...
                      foreach($recentProjects as $recentProject){ ?>
                            ...
                            <td class="tbl-ttl"><span class="onmobile"><?php echo $lang['Title']; ?>: </span> <?php echo $recentProject->idtype;?></td>
							  ...
							  <ul>
								...
								<li>
								<form method="post" action="edit-categories-agenda.php">
