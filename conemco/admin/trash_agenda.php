...
if(isset($_POST['del_proj']))
{
	 $delProjId=$_POST['del_id'];
	  ...
	  $deleteProj="delete from customers where idcustomer=$delProjId limit 1";
	  $projDeleted=mysqli_query($connect, $deleteProj);
	  $projChatDeleted=mysqli_query($connect, $deleteProjChat);
	  if($projDeleted){
		 header("Location:trash_agenda.php?message=trash");
	   } else{
			   header("Location:trash_agenda.php?message=fail");
	   }
 }
if(isset($_POST['comp_proj']))
{
	 ...
	 $comp_val=$_POST['comp_val'];
	  $updateProj="UPDATE customers SET status=$comp_val WHERE idcustomer=$comp_id";
	  $comp_proj=mysqli_query($connect, $updateProj);
if($comp_proj){
		  header("Location:agenda.php?message=restore");
	  } else{
...
if(isset($_POST['arc_proj']))
{
	 ...
	 $arc_del_val=$_POST['arc_del_val'];
	  $updateProj="UPDATE customers SET trash=$arc_del_val WHERE idcustomer=$arc_id";
...
$notmsgb = $lang['Client has been created successfully!']; 
$notmsgc = $lang['Error! Please Try Again later.'];
$notmsgd = $lang['Client has been deleted sucessfully'];
$notmsgg = $lang['Client status updated to re-open.'];
$notmsgh = $lang['Client restored Successfully.'];
...
<?php include('../templates/top-header.php'); ?>
         <div class="row">
			...
			<div class="row project-dash">
				...
				<div class="pm-heading"><h2><?php echo $lang['Trash']; ?> </h2><span><?php echo $lang['All client deleted']; ?></span></div>
				<div class="pm-form"><form class="form-inline md-form form-sm">
    <input class="form-control form-control" type="text" placeholder="<?php echo $lang['Search client']; ?>" id="protbl-input">
...
<?php $recentProjects=status::findBySql("select * from customers WHERE trash=1");?>
				...
				<div class="projbtnm"><a href="add-new-agenda.php"><span>+</span></a></div>
				...
				<li class="cproject"><a href="add-new-agenda.php"><?php echo $lang['Create client']; ?> <span>+</span></a></li>
				<li class="pm-arc"><a href="agenda.php"><?php echo $lang['client']; ?></a></li>
				<li class="pm-trashbox"><a href="trash_agenda.php">
				<?php echo $lang['TRASH']; ?> (<?php echo count($recentProjects); ?>)
				</a></li>
				</ul>
                ...
                <table class="table table-new projectspage" data-pagination="true" data-page-size="5">
                        <thead>
                          <tr>
                            ...
                            <th width="26%"><?php echo $lang['client Name']; ?></th> 
							<th><?php echo $lang['phone']; ?></th>
                            <th><?php echo $lang['email client']; ?></th>
					...
					if($recentProjects == NULL){
						  echo '<tr><td colspan="9">'.$lang['There is no client Please Create client!'].'</td></tr>';
					  }
                      foreach($recentProjects as $recentProject){ ?>
                      
                            <tr>
							<td class="bs-checkbox" style="width:36px !important; text-align:left !important;"><input data-index="<?php echo $counter; ?>" value="<?php echo $recentProject->idcustomer ?>" name="btSelectItem" type="checkbox"></td>
                            <td class="tbl-ttl"><span class="onmobile"><?php echo $lang['Title']; ?>: </span> <?php echo $recentProject->name;?></td>

                              <td><span class="onmobile"><?php echo $lang['Title']; ?>: </span> <?php echo $recentProject->phone;?></td>
                           
                             
                              <td><span class="onmobile"><?php echo $lang['Deadline']; ?>: </span> <?php echo $recentProject->email;?></td>
							  ...
							  <div class="action-toggle" data-toggle="collapse" data-target="#trash-menu<?php echo $recentProject->idcustomer;?>"><?php echo $lang['Action']; ?> <i class="fa fa-caret-down"></i></div>
							  <div id="trash-menu<?php echo $recentProject->p_id;?>" class="toggle-action collapse">
							  <ul>
								<li>
								<form method="post" action="#">
								<input type="hidden" value="<?php echo $recentProject->idcustomer;?>" name="arc_id"/>
								...
								<li>
								<button type="button" data-toggle="modal" class="mod" data-target="#deletepro<?php echo $recentProject->idcustomer;?>"><i class="fa fa-trash-o"></i> <?php echo $lang['Delete Permanently']; ?></button>
								</li>
							</ul>
								</div>
							...
							<!-- The Modal -->
<div  class="modal hide fade deletepro"  tabindex="-1" aria-labelledby="basicModal" aria-hidden="true" id="deletepro<?php echo $recentProject->idcustomer;?>" role="dialog">
...
<div class="mod-footbox">
<form method="post" action="#">
								<input type="hidden" value="<?php echo $recentProject->idcustomer;?>" name="del_id"/>
...
$(document).ready(function() {
	$(".mod").click(function(){
		$('#deletepro<?php echo $recentProject->idcustomer;?>').modal('toggle');
		$("#deletepro<?php echo $recentProject->idcustomer;?>").removeClass("fade");
