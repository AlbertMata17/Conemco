...
$account_stat=$user->status;
	$pro_id =  $_POST['idtype'];
if($pro_id == ""){
	header("Location:type_projects.php");
}
...
$flag=0;//determines if all posted values are not empty includ
		...
		if($flag==0)
		{
			...
			$typeTite = mysqli_real_escape_string($connect, $_POST['typeTite']);
				 $pp_id	= $pro_id;
				$pp_title	= $typeTite;
...
$sql_up = "UPDATE `project_types` SET
`name`='$pp_title',
`description`='$pp_desc',
`status`='$p_status',
		...
		$variablesArr = array('{USER_NAME}' => $user->firstName, '{SIGNATURE}' => $company_name, '{DASHBOARD_URL}' => $url, '{PROJECT_NAME}' => $pp_title);
						...
						$message = strtr($templateHTML, $variablesArr);
						  ...
						  if($emailSent){ 
							...
							header("Location:type_projects.php?message=error_email");
						 }
				}
				}
 }
...
header("Location:type_projects.php?message=success");
				} else {
...
<?php 
$qur_pro = Type::findBySql("select * from project_types where idtype = '$pro_id'");
...
<div class="col-md-8">
			<div class="form-group">
			 <div class="field-label"><label for="firstName"><?php echo $lang['type title']; ?></label></div>
				<input type="text" name="typeTite" class="form-control" value="<?php echo $name; ?>" required>
			</div>
			<div class="form-group">
			<div class="field-label"><label for="firstName"><?php echo $lang['Write a type description here']; ?></label></div>
...
<div class="inline field">
	...
	<input type="checkbox" name="notifyClient" tabindex="0" class="hidden"/>
      <label><?php echo $lang['Email Notification']; ?><br><span><?php echo $lang['Notify to client and staff type has been Updated']; ?></span></label>
    </div>
<input type="hidden" value="<?php echo $pro_id_u;?>" name="idtype"/>
<input type="submit" name="update-type" value="<?php echo $lang['Update type']; ?>"  class="btn new-btnblue"/>
</div>
</div>
</div>			
</div> 
