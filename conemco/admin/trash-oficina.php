...
if(isset($_POST['del_proj']))
{
	 $delProjId=$_POST['del_id'];
	  ...
	  $deleteProj="delete from office where idoficina=$delProjId limit 1";
	  $projDeleted=mysqli_query($connect, $deleteProj);
	  $projChatDeleted=mysqli_query($connect, $deleteProjChat);
	  if($projDeleted){
		 header("Location:trash-oficina.php?message=trash");
	   } else{
			   header("Location:trash-oficina.php?message=fail");
	   }
 }
if(isset($_POST['comp_proj']))
{
	 ...
	 $comp_val=$_POST['comp_val'];
	  $updateProj="UPDATE office SET status=$comp_val WHERE idoficina=$comp_id";
	  $comp_proj=mysqli_query($connect, $updateProj);
if($comp_proj){
		  header("Location:oficina.php?message=restore");
	  } else{
...
if(isset($_POST['arc_proj']))
{
	 ...
	 $arc_del_val=$_POST['arc_del_val'];
	  $updateProj="UPDATE office SET trash=$arc_del_val WHERE idoficina=$arc_id";
...
$notmsgb = $lang['office has been created successfully!']; 
$notmsgc = $lang['Error! Please Try Again later.'];
$notmsgd = $lang['office has been deleted sucessfully'];
$notmsgg = $lang['office status updated to re-open.'];
$notmsgh = $lang['office restored Successfully.'];
...
<?php include('../templates/top-header.php'); ?>
         <div class="row">
			...
			<div class="row project-dash">
				...
				<div class="pm-heading"><h2><?php echo $lang['Trash']; ?> </h2><span><?php echo $lang['All office deleted']; ?></span></div>
				<div class="pm-form"><form class="form-inline md-form form-sm">
    <input class="form-control form-control" type="text" placeholder="<?php echo $lang['Search office']; ?>" id="protbl-input">
    <button type="submit"><i class="fa fa-search" aria-hidden="true"></i></button>
</form></div>
<?php $recentProjects=Oficina::findBySql("select * from office WHERE trash=1");?>
				</div> 
                <div class="col-lg-7 col-md-7 col-sm-7 project-opt creative-right text-right">
				<div class="mobilepromenu">
				<div class="projbtnm"><a href="add-oficina.php"><span>+</span></a></div>
				<div class="projmobilem">
				<i class="fa fa-bars" aria-hidden="true"></i>
					<ul>
					<li class="pm-trashbox"><a href="trash-oficina.php">
...
<li class="cproject deskvisibleb"><a href="add-oficina.php"><?php echo $lang['Create office']; ?> <span>+</span></a></li>
<li class="cproject tabvisible"><a href="add-oficina.php"><span>+</span></a></li>
<li class="pm-trashbox"><a href="trash-oficina.php">
				<?php echo $lang['TRASH']; ?> (<?php echo count($recentProjects); ?>)
				</a></li>
				<li class="pm-trash">
...
$recentProjects_page=Oficina::findBySql("select * from office WHERE trash = 1"); 
$recentProjects=Oficina::findBySql("select * from office WHERE trash = 1 ORDER BY date_created DESC LIMIT $start_from, $limit"); 
                ...
                <table class="table table-new projectspage" data-pagination="true" data-page-size="5">
                        <thead>
                          <tr>
                            ...
                            <th width="26%"><?php echo $lang['office Name']; ?></th> 
                            <th><?php echo $lang['Description office']; ?></th>
					...
					if($recentProjects == NULL){
						  echo '<tr><td colspan="9">'.$lang['There is no office Please Create office!'].'</td></tr>';
					  }
                      foreach($recentProjects as $recentProject){ ?>
                      
                            <tr>
							<td class="bs-checkbox" style="width:36px !important; text-align:left !important;"><input data-index="<?php echo $counter; ?>" value="<?php echo $recentProject->idoficina ?>" name="btSelectItem" type="checkbox"></td>
                            <td class="tbl-ttl"><span class="onmobile"><?php echo $lang['Title']; ?>: </span> <?php echo $recentProject->name;?></td>

                              <td><span class="onmobile"><?php echo $lang['Title']; ?>: </span> <?php echo $recentProject->description;?></td>
                           
                             
                              <td><span class="onmobile"><?php echo $lang['Deadline']; ?>: </span> <?php echo $recentProject->contact;?></td>
                              <td><span class="onmobile"><?php echo $lang['Deadline']; ?>: </span> <?php echo $recentProject->address;?></td>
                              <td><span class="onmobile"><?php echo $lang['Deadline']; ?>: </span> <?php echo $recentProject->phone;?></td>
							  ...
							  <div class="action-toggle" data-toggle="collapse" data-target="#client-menu<?php echo $recentProject->idoficina;?>"><?php echo $lang['Action']; ?> <i class="fa fa-caret-down"></i></div>
							  <div id="client-menu<?php echo $recentProject->idoficina;?>" class="toggle-action collapse">
							  <ul>
								<li>
								<form method="post" action="#">
								<input type="hidden" value="<?php echo $recentProject->idoficina;?>" name="arc_id"/>
								...
								<li>
								<button type="button" data-toggle="modal" class="mod" data-target="#deletepro<?php echo $recentProject->idoficina;?>"><i class="fa fa-trash-o"></i> <?php echo $lang['Delete Permanently']; ?></button>
								</li>
							</ul>
								</div>
							...
							<!-- The Modal -->
<div  class="modal hide fade deletepro"  tabindex="-1" aria-labelledby="basicModal" aria-hidden="true" id="deletepro<?php echo $recentProject->idoficina;?>" role="dialog">
...
<div class="mod-footbox">
<form method="post" action="#">
								<input type="hidden" value="<?php echo $recentProject->idoficina;?>" name="del_id"/>
...
$(document).ready(function() {
	$(".mod").click(function(){
		$('#deletepro<?php echo $recentProject->idoficina;?>').modal('toggle');
		$("#deletepro<?php echo $recentProject->idoficina;?>").removeClass("fade");
